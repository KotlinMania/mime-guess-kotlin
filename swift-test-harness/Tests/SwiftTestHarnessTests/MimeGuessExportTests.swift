...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `MimeGuess.swiftmodule/`
...
//      `import MimeGuess` above would fail at compile time.
//
//   2. The static archive `libMimeGuess.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "MimeGuess" }` and
//      `flattenPackage = "io.github.kotlinmania.mimeguess"` configuration in
...
//      via the `MimeGuessLibrary` product.
//
...
final class MimeGuessExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "MimeGuess swift module imported cleanly")
    }
}

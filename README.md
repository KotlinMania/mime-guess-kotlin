...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`mime_guess`](https://crates.io/crates/mime_guess).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `mime_guess` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

Guess MIME type from a path's extension
...
Original work copyrighted by the upstream `mime_guess` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.

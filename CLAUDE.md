# Claude Code Project Instructions — mime-guess-kotlin

## Project Overview

This is **mime-guess-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`mime_guess`](https://crates.io/crates/mime_guess).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/mime_guess/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/mime_guess>
